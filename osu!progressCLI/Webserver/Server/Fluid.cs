...
namespace osu_progressCLI.Webserver.Server
{
    ...
    {
        public static readonly List<KeyValuePair<string, IFluidTemplate>> templates =
            new List<KeyValuePair<string, IFluidTemplate>>();
        ...
        public static async Task setup(FluidParser parser)
        {
            ...
            IFileProvider fileProvider = new PhysicalFileProvider(
                Path.GetFullPath("Webserver/Fluid")
            );
            ...
            foreach (string filename in files)
            {
                if (
                    !parser.TryParse(
                        await File.ReadAllTextAsync(DEFAULTFOLDER + filename),
                ...
                )
                {
                    throw new InvalidOperationException(
                        ...
                        new FileNotFoundException(
                            "The specified template file was not found or could not be read.",
                            filename
                        )
                    );
                }
                ...
                Logger.Log(
                    ...
                    $"{filename} succsefully parsed and added!"
                );
            }
        ...
        public static async Task<string> RenderTemplateAsync<T>(
            ...
            where T : class
        {
            if (
                !parser.TryParse(
                    await File.ReadAllTextAsync(DEFAULTFOLDER + staticFilePath),
                    out var template,
                    out var error
                )
            )
        ...
        public static async Task<string> RenderTemplateListAsync<T>(
            string staticFilePath,
            T data,
            FluidParser parser
        )
            where T : class
        {
            if (
                !parser.TryParse(
                    await File.ReadAllTextAsync(DEFAULTFOLDER + staticFilePath, Encoding.UTF8),
                    out var template,
                    out var error
                )
            )
