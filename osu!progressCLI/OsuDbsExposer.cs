        ...
        static void ParseDBs()
        {
            ...
            if (File.Exists(filepath + file))
            {
                using (FileStream fileStream = new FileStream(filepath + file, FileMode.Open, FileAccess.Read, FileShare.ReadWrite, bufferSize: 4096, useAsync: true))
                {
                    ...
                    Osudb = OsuParsers.Decoders.DatabaseDecoder.DecodeOsu($"{Credentials.Instance.GetConfig().osufolder}{file}");
            ...
            if (File.Exists(filepath + file))
            {
                ...
                {
                    Console.WriteLine($"Parsing {file}");
                    Presencedb = OsuParsers.Decoders.DatabaseDecoder.DecodePresence($"{filepath}{file}");
                    Console.WriteLine($"Parsed {file}");
                }
        ...
        public static void MatchallScorestobeatmaps()
        {
            Scoredb.Scores.ForEach(s => {
                ...
                if(beatmap != null)
                {
                    ...
                    {
                        Console.WriteLine($"{beatmap.Title} : {score.ScoreId} O {score.BeatmapMD5Hash}");
                    }
                }
            });
