    ...
    public sealed class ApiController
    {
        ...
        private async Task<JObject> MakeRequest(string Query)
        {
            ...
            if(login is not null)
            {
                ...
                {
                    Logger.Log(Logger.Severity.Debug, Logger.Framework.Network, $"Saved Token No Longer Valid, Requesting new Token");
            ...
            try
            {
                ...
                if (reponse.IsSuccessStatusCode)
                {
                    ...
                    Logger.Log(Logger.Severity.Info, Logger.Framework.Network, $"Recived Answer for: {Query}");

                    return JObject.Parse(responseBody);
                }
                else
                {
                    Logger.Log(Logger.Severity.Warning, Logger.Framework.Network, $"Request for {Query} Failed: {reponse.StatusCode}");
        ...
        private async void getAccessToken()
        {
            ...
            {
            new KeyValuePair<string, string>("client_id", Credentials.Instance.GetClientId()),
            new KeyValuePair<string, string>("client_secret", Credentials.Instance.GetClientSecret()),
        ...
        public async Task getAccessToken(string refresh_token)
        {
            ...
            string oauthTokenEndpoint = $"https://osu-progress-oauth-helper.vercel.app/refresh?port={Credentials.Instance.GetConfig().port}&refresh_token={refresh_token}";
            ...
            try
            {
                ...
                {
                    Logger.Log(Logger.Severity.Warning, Logger.Framework.Network, $"Request for Failed: {reponse.StatusCode}");
            ...
            {
                Logger.Log(Logger.Severity.Warning, Logger.Framework.Network, $"Request for Failed: {ex.Message}");
            }
            finally
            {
                client.Dispose();
