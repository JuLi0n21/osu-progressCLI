...
function saveSettings() {
  ...
  const localConfigEnabled = document.getElementById(
    "localsettingstoggle"
  ).checked;
  ...
  fetch("api/save", {
    ...
    .then((data) => {
      console.log(data);
      alert(
        "Settings saved, to Avoid overloading the Osu api will users only be refreshed every 5 minutes (or restart the program)"
      );
      location.reload();
    })
    .catch((error) => {
