    ...
    if (jsonData.Type === "baseaddresses") {
      ...
      if (audioTime === lastAudioTime) {
        ...
        content = `<div class="flex justify-center"> 
                    <div class="flex">
                        <div class="flex">
                            <pre class="text--yellow"> ${jsonData.Data.Player.Accuracy.toFixed(
                              2
                            )}%</pre>
                            <pre class="text-green-500"> ${
                              jsonData.Data.Player.Hit100
                            } </pre>
                            <pre class="text--orange"> ${
                              jsonData.Data.Player.Hit50
                            } </pre>
                            <pre class="text-red-500"> ${
                              jsonData.Data.Player.HitMiss
                            } </pre>
                            <pre class="text--ping"> ${
                              jsonData.Data.Player.Score
                            } </pre>
                       </div>
                    </div>
                </div>`;
      ...
      audioTextElement.innerHTML = `
            <div class="flex justify-between">
                <div class="justify-self-start">${Math.floor(
                  audioTime / 1000 / 60
                )}:${Math.floor((audioTime / 1000) % 60)
        .toString()
        .padStart(2, "0")
        .slice(0, 3)}  </div>
                <div class="justify-self-end">${Math.floor(
                  totalAudioTime / 1000 / 60
                )}:${Math.floor((totalAudioTime / 1000) % 60)
        .toString()
        .padStart(2, "0")
        .slice(0, 3)}  </div>
            </div>`;
