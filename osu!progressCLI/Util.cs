...
public class Util
{
    ...
    )
    {
        Logger.Log(
            ...
            $"Getting Background from: {folderpath}/{osufile}"
        );
        ...
        if (File.Exists(filepath))
        {
            ...
            if (match.Success)
            {
                ...
                Logger.Log(
                    Logger.Severity.Debug,
                    Logger.Framework.Misc,
                    $"Found Background Image: {background}"
                );
        ...
        try
        {
            Logger.Log(
                Logger.Severity.Debug,
                Logger.Framework.Misc,
                $"No Background Image Found Finding Backup!"
            );
            string bgDirectory = Path.Combine(
                Credentials.Instance.GetConfig().osufolder,
                "Data/bg"
            );
            string btDirectory = Path.Combine(
                Credentials.Instance.GetConfig().osufolder,
                "Data/bt"
            );
            ...
            if (bgFiles.Length > 0)
            {
                ...
                File.Copy(
                    Path.GetFullPath(bgFiles[index]),
                    $"public/img/{Path.GetFileName(bgFiles[index])}",
                    true
                );
            ...
            else if (btFiles.Length > 0)
            {
                ...
                File.Copy(
                    Path.GetFullPath(btFiles[index]),
                    $"public/img/{Path.GetFileName(btFiles[index])}",
                    true
                );
    ...
    public static string osufile(string foldername, string version, string fullparentpath = null)
    {
        ...
        if (fullparentpath == null)
        {
            files = Directory.GetFiles(
                $"{Credentials.Instance.GetConfig().songfolder}/{foldername}"
            );
        }
        else
        {
